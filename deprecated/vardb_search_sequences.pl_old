...
while (<IN>) {
	...
	if ($do_blast) {
		...
		system "extract_fasta.pl -d $seed -i $GENOMEDB/$organism/$organism-protein.idx > $seedfile";
	
		if ($search_genome) {
			$base = "$family-$organism";
			# search in protein database with psi-blast and generate pssm file.
			system "blastpgp -d $GENOMEDB/$organism/$organism-protein -i $seedfile -s T -j $iter -h $pssm_eval -C $base.chk -F T -b 10000  > $base.blastpgp";
			...
			system "blastall -p psitblastn -d $GENOMEDB/$organism/$organism-nucleotide -i $seedfile -R $base.chk -b 10000 > $base.psitblastn";
			...
			# read position file.
			my $pos = new varDB::Position({file => "$GENOMEDB/$organism/$organism-position.txt", format => $format});
			
			# read list file.
			my $seqs = parse_list_file("$base-protein.list");
			my $nseqs = $seqs->{nseq};
			
			# quality check: check number of exons.
			open OUTLIST, ">$base-protein.list";
			...
			print NUMBER "$nseqs\t$family\t$organism\tprotein\n";
	...
	if ($do_hmmer) {
		...
		my $fs = $hmm_name."_ls";
		my $ls = $hmm_name."_fs";
		system "hmmfetch $HMMDB/Pfam_ls $hmm_name > $fs.hmm";
		system "hmmfetch $HMMDB/Pfam_fs $hmm_name > $ls.hmm";
		
		if ($search_genome) {
			$base = "$family-$organism";
			# search in protein genome.
			system "hmmsearch $ls.hmm $GENOMEDB/$organism/$organism-protein.fa > $base\_ls.log";
			system "hmmsearch $fs.hmm $GENOMEDB/$organism/$organism-protein.fa > $base\_fs.log";
